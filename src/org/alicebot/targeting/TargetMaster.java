...
public class TargetMaster extends Targeting
{
    ...
    static {
        if (Globals.useTargeting())
        {
            ...
            TARGETS_DATA_RESOURCE.encoding =
                Globals.getProperty(
                    "programd.targeting.data.encoding",
                    "UTF-8");
    ...
    public static void rollTargetData()
    {
        if (Globals.useTargeting())
        {
            ...
            targetsData = FileManager.getFile(TARGETS_DATA_PATH);
            Log.userinfo(
                "Targeting data deleted (old file rolled over).",
                Log.TARGETING);
        }
        else
        {
            Trace.userinfo(
                "Targeting is switched off. Turn it on in server.properties.");
        }
    }
}