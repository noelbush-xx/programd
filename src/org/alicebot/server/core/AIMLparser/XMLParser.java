...
public class XMLParser extends Object {
        ...
        */
        ...
        /**
         ...
         It's really a Poor's man XML non-validating parser.

         It should be replaced by any decent standard XML parser
         object anytime soon after the migration.

         It's inefficient to perform a XML parsing here and
         another (with different methods and algorithms) on
         AIMLReader.java; however the inconsistency is accepted
         because when a real XML parser object is adopted both
         should go, I have a real need to parse XML here and
         a zero need to touch AIMLReader.java at this point.

         It somewhat lacks sense to replace one and
         not the other and for me it lacks sense to mess up
         with AIMLReader.java too.

         Actually, to process XML here is hugely inefficient
         since for each template processed the XML of it will
         be parsed over and over again. The real mod should be
         to parse the XML on the template once during loading
         and store a trie representation of it instead of the
         actual text string; then the parser would just scan
         the trie without needing to build it in the first place.

         However, this would take an huge time to test mainly
         and we're in short supply of time at this point, so..

         Also, a XML parser object hasn't been used at this point in
         order to expedite the migration, substantial architectural
         discussion should be made prior to adopt any specific
         parser object. Also, a full XML compliant validation process
         will highlight some problems with AIML 1.0 due to
         conflicts with the XML normalization process
         (i.e. <tag1/> <tag2/> ==> <tag1/><tag2/>) for which
         additional facilities are needed (i.e. &x; tokens) and of
         course time to discuss them.

         At least this XML parser is rather fast and have a very
         reduced memory footprint, but has to be changed into
         some more standard down the road.

         A nice gift to anyone moving this beast into AIML 1.x
         (with x != 0); my mother is rather unaware of my whereabouts
         as a weekend Java programmer so please don't involve her
         on your thoughts.
