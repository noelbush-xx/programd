...
public class AIMLParser extends Object {
	...
	public static String getArg(String argname, String args) {
          ...
          */
           ...
           */
           ...
           /*
             <get name="varname"/>
           */
             if ( (tag.XMLData.equals(AIML10Tag.GET)) && (tag.XMLType == tag.EMPTY) ) {
                p = new GetProcessor();
                return p.processAIML(level++,ip,tag,this);
             }
        ...
        */
        ...
        /**
          This method is for debug purposes only and must go
        */
	public static void main(String[] args)
	{
                String teststr = "StArT<li  >My Id is <id/> <version/> <size/> <!-- Test --><date/><li name=\"xxx\" value=\"yyyy\">TEST1<li>Test2<![CDATA[TeSt3]]><get name=\"xxxx\"/><think>hello</think><br /></li></li></li  >end";
                String ip      = "127.0.0.1";
                AIMLParser p;

                p = new AIMLParser();

                teststr = "My Id is <id/> Date is <date/> Version is <version/> Size is <size/>";
                String response = p.processResponse(ip,teststr);
                System.out.println("*** RESPONSE: ("+response+") ***");

                teststr = "nesting <li><li><li>Hello</li></li></li>";
                response = p.processResponse(ip,teststr);
                System.out.println("*** RESPONSE: ("+response+") ***");

                teststr = "Now you see it (<think>Now you don't</think>)";
                response = p.processResponse(ip,teststr);
                System.out.println("*** RESPONSE: ("+response+") ***");

                teststr = "<lowercase>LOWERCASE</lowercase> <uppercase>uppercase</uppercase> <formal>this is a formal phrase</formal> <sentence>this is a sentence</sentence>";
                response = p.processResponse(ip,teststr);
                System.out.println("*** RESPONSE: ("+response+") ***");
