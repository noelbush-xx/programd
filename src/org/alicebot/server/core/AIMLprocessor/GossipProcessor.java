...
public class GossipProcessor implements AIMLProcessor, Serializable {
         ...
         */
         ...
         /*
           If the src= attribute has been informed then the file might
           be different
         */
         if (tag.XMLAttr.toLowerCase().indexOf("src=",0) >= 0) {
            String Logsrc = p.getArg("src",tag.XMLAttr.toLowerCase());
            if (!Logsrc.equals("")) {
               LogName = Log.LOGPATH + "/" + Logsrc;
            }
         }
