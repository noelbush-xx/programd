...
class SelfTest {
  ...
  static void test(int max,boolean persistent) {
    ...
    try {
      DriverManager.registerDriver(new org.alicebot.server.sql.jdbcDriver());
      if(persistent) {
        delete("test2.backup");
        delete("test2.properties");
        delete("test2.script");
        delete("test2.data");
        test("Persistent","jdbc:alicebot:test2","dany","",max,true);
      }
      test("In-Memory","jdbc:alicebot:.","dany","",max,false);
