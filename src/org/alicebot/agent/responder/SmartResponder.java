...
package org.alicebot.agent.responder;
...
import org.alicebot.multiplexor.Multiplexor;
...
public class SmartResponder
{
    ...
    private static final String INACTIVITY =
        Globals.getProperty("programd.inactivity-string", "INACTIVITY");
    ...
    /** The name of the response encoding parameter in a request (&quot;response_encoding&quot;). */
    ...
    private static final String[] HTML_USER_AGENTS =
        new String[] { "Mozilla", "MSIE", "Lynx", "Opera" };
        ...
        HttpServletResponse response)
    {
        ...
        else
        {
            ...
            {
                this.userRequest =
                    new String(this.userRequest.getBytes(ENC_8859_1), ENC_UTF8);
            }
    ...
    public void doResponse()
    {
        ...
        catch (IOException e)
        {
            throw new DeveloperError(
                "Error getting service response output stream.",
                e);
        }
            ...
            case ServiceType.HTML :
                ...
                if (this.botResponse == null)
                {
                    this.botResponse =
                        Multiplexor.getResponse(
                            this.userRequest,
                            (String) this.serviceRequest.getSession(
                                true).getAttribute(
                                HTMLResponder.USER_COOKIE_NAME),
                            this.botid,
                            responder);
                }
                break;
        ...
        {
            throw new UserError(
                "UTF-8 encoding is not supported on your platform!",
                e0);
        }
        catch (IOException e1)
        {
            throw new DeveloperError(
                "Error writing to service output stream.",
                e1);
        }
        try
        {
        ...
        {
            throw new DeveloperError(
                "Error flushing service output stream.",
                e);
        }
        try
        {
    ...
    public int getServiceType()
    {
        ...
        for (int index = HTML_USER_AGENT_COUNT; --index >= 0;)
        {
            if (this
                ...
                .indexOf(HTML_USER_AGENTS[index])
                != -1)
            {
