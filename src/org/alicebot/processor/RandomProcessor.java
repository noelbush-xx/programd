...
import org.alicebot.util.LRUCache;
import org.alicebot.util.MersenneTwisterFast;
...
public class RandomProcessor extends AIMLProcessor
{
        ...
        throws AIMLProcessorException
    {
        if (tag.XMLType == XMLNode.TAG)
        {
            ...
            String identifier =
                parser.getBotID() + parser.getUserID() + tag.toString();

            // Does the generators map already contain this one?
            MersenneTwisterFast generator =
                (MersenneTwisterFast) generators.get(identifier);
            if (generator == null)
            {
            ...
            if (nodeCount == 1)
            {
                return parser.evaluate(
                    level++,
                    parser.getNode(LI, tag.XMLChild, 1).XMLChild);
            }
