...
/**
 * A ChatLogEvent contains additional information about an exchange in a chat. Note that a ChatLogEvent must be logged
 * using {@link Logger#callAppenders(LoggingEvent)}, not one of the wrapper methods like "info" nor the basic "log"
 * method. This is due to a limitation with log4j (the provided logging methods will re-cast the event as a LoggingEvent
...
public class ChatLogEvent extends LoggingEvent {
  ...
  public ChatLogEvent(String bot, String user, String in, String out) {
    super(LOGGER_FQCN, Logger.getLogger("programd." + bot), Level.INFO, String.format(
        "%s -> %s: \"%s\"; %s -> %s: \"%s\"", user, bot, in, bot, user,
        Text.normalizeString(Characters.removeMarkup(out))), null);
    this.botid = bot;
    this.userid = user;
    this.input = in;
    this.reply = out;
