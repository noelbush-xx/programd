...
public class Shell extends Thread {
  ...
  /**
   * A <code>Shell</code> with default input and output streams ( <code>System.in</code> and <code>System.out</code>).
   * 
   */
  public Shell() {
    super("Shell");
  ...
  public void run() {
    ...
    this.showMessage(String.format("Interactive shell: type \"/exit\" to shut down; \"%s\" for help.",
        HelpCommand.COMMAND_STRING));
        ...
        if (commandLine.indexOf('#') == 0) {
          ...
          try {
            ...
            try {
              ...
              this.showError(String.format("Error processing command: \"%s\"", Errors.describe(e)));
            }
          }
          catch (NoSuchCommandException e) {
            // May be a commandable.
            try {
              this.commandCommandable(commandLine);
            }
            catch (NoCommandException ee) {
              this.showError("Please specify a command following the commandable.  For a list of commandables, type \""
                  + ListCommandablesCommand.COMMAND_STRING + "\".");
            }
            catch (NoSuchCommandableException ee) {
              this.showError("No such commandable is loaded.");
            }
          }
        }
        else if (commandLine.length() > 0) {
          this.showConsole(this.botName,
              XHTML.breakLines(this._core.getResponse(commandLine, this.hostname, this.botid)));
        }
  ...
  protected void showConsole(String preprompt, String[] message) {
    for (String element : message) {
      this.printlnOut(String.format("%s> %s", preprompt, element));
    }
  }
  ...
  protected void showPrompt() {
    if (this.getState() != Thread.State.NEW) {
      this.promptConsole('[' + this.botName + "] "
  ...
  public void switchToBot(String newBotID) {
    if (!this.bots.containsKey(newBotID)) {
      this.showError("That bot id is not known. Check your startup files.");
      return;
    }
    this.botid = newBotID;
    this.botName = this.bots.get(newBotID).getPropertyValue(this.botNamePredicate);
    this.showMessage("Switched to bot \"" + newBotID + "\" (name: \"" + this.botName + "\").");
    // Send the connect string and print the first response.
    this.showConsole(this.botName, XHTML.breakLines(this._core.getResponse(this._core.getSettings().getConnectString(),
        this.hostname, this.botid)));
  }
}
