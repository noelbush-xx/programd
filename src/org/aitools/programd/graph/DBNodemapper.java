...
public class DBNodemapper {
  ...
  protected static void associateBotWithFile(Connection connection, String bot, URL filename) {
    try {
      PreparedStatement insert = connection
          .prepareStatement("INSERT INTO `bot_file` (`bot_id`, `file_id`) VALUES (bot_id, file_id) WHERE bot_id = (SELECT `id` from `bots` WHERE `label` = ?) AND file_id = (SELECT `id` FROM `files` WHERE `path` = ?)");
  ...
  public static boolean containsKey(Connection connection, int node, String key) {
    ...
    try {
      PreparedStatement select = connection
          .prepareStatement("SELECT COUNT(*) FROM `edges` WHERE `from_node_id` = ? AND `label` = ?");
  ...
  public static boolean fileIsAlreadyPresent(Connection connection, URL file) {
    int count = 0;
    try {
      PreparedStatement select = connection.prepareStatement("SELECT COUNT(*) FROM `files` WHERE `path` = ?");
  ...
  public static boolean fileIsAlreadyPresentForBot(Connection connection, URL file, String bot) {
    ...
    try {
      PreparedStatement select = connection
          .prepareStatement("SELECT COUNT(*) FROM `files` INNER JOIN `bot_file` ON `files`.`id` = `bot_file`.`file_id` INNER JOIN `bot_file` ON `bots`.`id` = `bot_file`.`bot_id` WHERE `files`.`path` = ? AND `bots`.`label` = ?");
  ...
  public static int get(Connection connection, int node, String key) {
    ...
    try {
      PreparedStatement select = connection
          .prepareStatement("SELECT `to_node_id` FROM `edges` WHERE `from_node_id` = ? AND `label` = ?");
      select.setInt(1, node);
      select.setString(2, key);
      ResultSet results = select.executeQuery();
      if (results.next()) {
  ...
  public static Set<Integer> getBotIDNodesForFile(Connection connection, URL file) {
    ...
    try {
      PreparedStatement select = connection
          .prepareStatement("SELECT `botidnode` from `botidnode_file` INNER JOIN `botidnode_file` ON `files`.`path` = ?");
      select.setString(2, file.toExternalForm());
      ResultSet results = select.executeQuery();
      while (results.next()) {
  ...
  public static List<String> getBotsForFilename(Connection connection, URL filename) {
    ...
    try {
      PreparedStatement select = connection
          .prepareStatement("SELECT `bots`.`label` FROM `bot_file` INNER JOIN `files` ON `bot_file`.`file_id` = `files`.`id` INNER JOIN `bots` ON `bots`.`id` = `bot_file`.`id` WHERE `files`.`path` = ?");
  ...
  public static List<String> getFilenames(Connection connection, int node) {
    ...
    try {
      PreparedStatement select = connection
          .prepareStatement("SELECT `files`.`path` from `file_node` INNER JOIN `files` ON `file_node`.`file_id` = `files`.`id` WHERE `file_node`.`node_id` = ?");
      select.setInt(1, node);
      ResultSet results = select.executeQuery();
      while (results.next()) {
        result.add(results.getString(1));
  ...
  public static List<URL> getFilenamesForBot(Connection connection, String bot) {
    ...
    try {
      PreparedStatement select = connection
          .prepareStatement("SELECT `files`.`path` FROM `bot_file` INNER JOIN `files` ON `bot_file`.`file_id` = `files`.`id` INNER JOIN `bots` ON `bot_file`.`bot_id` = `bots`.`id` WHERE `bots`.`id` = (SELECT `id` FROM `bots` WHERE `label` = ?)");
  ...
  public static long getLastLoaded(Connection connection, URL file) {
    long result = -1;
    try {
      PreparedStatement select = connection.prepareStatement("SELECT `last_loaded` FROM `files` WHERE `path` = ?");
      select.setString(1, file.toExternalForm());
      ResultSet results = select.executeQuery();
      if (results.next()) {
  ...
  public static int getParent(Connection connection, int node) {
    ...
    try {
      PreparedStatement select = connection
          .prepareStatement("SELECT `from_node_id` from `edges` WHERE `to_node_id` = ?");
      select.setInt(1, node);
      ResultSet results = select.executeQuery();
      if (results.next()) {
  ...
  public static String getTemplate(Connection connection, int node) {
    ...
    try {
      PreparedStatement select = connection
          .prepareStatement("SELECT `label` from `templates` INNER JOIN `node_template` ON `node_template`.`template_id` = `templates`.`id` WHERE `node_template`.`node_id` = ?");
      select.setInt(1, node);
      ResultSet results = select.executeQuery();
      if (results.next()) {
  ...
  public static int put(Connection connection, int node, String key) {
    ...
    try {
      ...
      if (!exists) {
        ...
        insert = connection
            .prepareStatement("INSERT INTO `edges` (`from_node_id`, `label`, `to_node_id`) VALUES (?, ?, ?)");
  ...
  public static void remove(Connection connection, int from_node, int to_node) {
    try {
      PreparedStatement delete = connection
          .prepareStatement("DELETE FROM `edges` WHERE `from_node_id` = ? AND `to_node_id` = ?");
  ...
  public static void removeBotIDFromFilename(Connection connection, String bot, URL filename) {
    try {
      PreparedStatement delete = connection
          .prepareStatement("DELETE FROM `botid_file` WHERE `bot_id` = (SELECT `id` FROM `bots` WHERE `label` = ?) AND `file_id` = (SELECT `id` FROM `files` WHERE `path` = ?)");
  ...
  public static void setTemplate(Connection connection, int node, String template) {
    int templateID = -1;
    try {
      PreparedStatement insert = connection.prepareStatement("INSERT INTO `templates` (`label`) VALUES (?)");
  ...
  public static int size(Connection connection, int node) {
    ...
    try {
      PreparedStatement select = connection
          .prepareStatement("SELECT COUNT(`from_node_id`) FROM `edges` WHERE `from_node_id` = ?");
      select.setInt(1, node);
      ResultSet results = select.executeQuery();
      if (results.next()) {
        count = results.getInt(1);
  ...
  public static void storeBotIDNodeFile(Connection connection, int node, URL file) {
    try {
      PreparedStatement insert = connection
          .prepareStatement("INSERT INTO `botidnode_file` (`botidnode_id`, `file_id`) VALUES (?, file_id) WHERE file_id = (SELECT `id` FROM `files` WHERE `path` = ?)");
      insert.setInt(1, node);
