...
public class Tester {
   ...
   */
  protected static HashMap<String, TestSuite> loadTests(List<URL> suiteList, Core core, Logger logger) {
  ...
  public Tester(Core core, Logger testLogger, List<URL> suitePaths, URL testReports) {
    ...
    try {
      this.testReportDirectory = Filesystem.checkOrCreateDirectory(testReports.getFile(), "test report directory")
  ...
  public String run(String botid, String suite, int runCount) {
    ...
    this.failures.clear();
    if (suite == null) {
      this.runAllSuites(botid, runCount);
    }
    else {
      this.runOneSuite(botid, suite, runCount);
    }
