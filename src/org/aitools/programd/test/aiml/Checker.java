...
abstract public class Checker {
  ...
  public static Checker create(Element element, String encoding) {
    ...
    if (tagName.equals(TAG_ALERT_KEYWORDS)) {
      try {
        return new AlertKeywordChecker(
            new String(Characters.unescapeXMLChars(element.getText()).getBytes(encoding)).intern());
      }
      ...
      try {
        return new ExpectedKeywordChecker(
            new String(Characters.unescapeXMLChars(element.getText()).getBytes(encoding)).intern());
      }
      catch (UnsupportedEncodingException e) {
        throw new DeveloperError(String.format("Platform does not support \"%s\" encoding!", encoding), e);
  ...
  /**
   * @return the result expected by the checker, or a description of it
   */
  abstract public String getExpected();
