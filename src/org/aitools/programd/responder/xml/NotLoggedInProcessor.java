 ...
 */
public class NotLoggedInProcessor extends XMLTemplateProcessor
{
    /** The string &quot;{@value}&quot;. */
    private static final String LOGGED_IN = "logged-in";

    /**
     * Creates a new NotLoggedInProcessor with the given Core.
     * 
     * @param coreToUse the Core to use in creating the NotLoggedInProcessor
     */
    public NotLoggedInProcessor(Core coreToUse)
    {
        super(coreToUse);
    }

    /** The label (as required by the registration scheme). */
    public static final String label = "not-logged-in";

    /**
     * @param element the <code>logged-in</code> element
     * @param parser the parser that is at work (unused)
     * @return the contents of this element, if the session is not logged in; otherwise, empty string
    ...
    public String process(Element element, XMLTemplateParser parser) throws XMLTemplateProcessorException
    {
        ...
        {
            throw new XMLTemplateProcessorException("Tried to use a <not-logged-in> element on a responder that is not servlet-aware!", new ClassCastException());
        }
        ...
        if (!responder.getEnvelope().sessionAttributeEquals(LOGGED_IN, true))
        {
            ...
            {
                throw new XMLTemplateProcessorException("Error processing content of <not-logged-in> element.", e);
            }
        }
        // otherwise...
        return EMPTY_STRING;
