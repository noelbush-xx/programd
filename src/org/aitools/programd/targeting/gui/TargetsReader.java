 ...
 *  This version is based on {@link org.aitools.programd.multiplexor.parser.AIMLReader}.
...
public class TargetsReader extends GenericReader implements Runnable
{
    ...
    */
     ...
     */
    private void transition(String tag, int toState, int action)
        ...
        if (succeed(tag, toState))
        {
            switch (action)
            {
                ...
                case SET_INPUT_CONTEXT :
                    ...
                    {
                        this.patternField =
                            this.getClass().getDeclaredField(INPUT_TEXT);
                        this.thatField =
                            this.getClass().getDeclaredField(INPUT_THAT);
                        this.topicField =
                            this.getClass().getDeclaredField(INPUT_TOPIC);
                ...
                case SET_MATCH_CONTEXT :
                    ...
                    {
                        this.patternField =
                            this.getClass().getDeclaredField(MATCH_PATTERN);
                        this.thatField =
                            this.getClass().getDeclaredField(MATCH_THAT);
                        this.topicField =
                            this.getClass().getDeclaredField(MATCH_TOPIC);
                        this.templateField =
                            this.getClass().getDeclaredField(MATCH_TEMPLATE);
                    }
                    catch (NoSuchFieldException e)
                    {
                        throw new DeveloperError("The developer has specified a field that does not exist in TargetsReader.");
                ...
                case SET_REPLY_CONTEXT :
                    try
                    {
                        ...
                        this.templateField =
                            this.getClass().getDeclaredField(REPLY);
                    }
                    catch (NoSuchFieldException e)
                    {
                        throw new DeveloperError("The developer has specified a field that does not exist in TargetsReader.");
