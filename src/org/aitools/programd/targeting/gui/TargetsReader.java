...
import org.aitools.programd.parser.GenericReader;
...
public class TargetsReader extends GenericReader implements Runnable
{
    ...
    /**
     * The most recently collected match &lt;pattern&gt;&lt;/pattern&gt;
     * contents.
     */
    ...
    /**
     * The most recently collected match &lt;template&gt;&lt;/template&gt;
     * contents.
     */
    ...
    /**
     * The progress monitor that will be maintained if this is created with a
     * parent component.
     */
    ...
    /**
     * The length of the file to be read (used if a progress monitor is
     * invoked).
     */
     ...
     * indicating that a <code>ProgressMonitor</code> should be created and
     * updated for that component.
     * 
     * @param parent
     ...
     */
    public TargetsReader(String fileNameToUse, BufferedReader buffReaderToUse, TargetsReaderListener targetsListener,
            String encodingToUse, long length, Component parent)
    {
        ...
        this.monitor = new ProgressMonitor(parent, "Reading targets from \"" + this.fileName + "\"", null, 0, 100);
    ...
    protected void tryStates() throws TransitionMade
    {
        ...
        switch (this.state)
        {
            ...
            case S_IN_TARGET:
                transition(Targeting.INPUT_START, TargetsReader.S_IN_INPUT, TargetsReader.SET_INPUT_CONTEXT);
                transition(Targeting.MATCH_START, TargetsReader.S_IN_MATCH, TargetsReader.SET_MATCH_CONTEXT);
            ...
            case S_IN_TEMPLATE:
                transition(Targeting.TEMPLATE_END, S_OUT_TEMPLATE, this.templateField);
                break;
        ...
        if (succeed(tag, toState))
        {
            switch (action)
            {
                case DELIVER_TARGET:
                    ...
                    ((TargetsReaderListener) super.listener).loadTarget(this.matchPattern, this.matchThat,
                            this.matchTopic, this.matchTemplate, this.inputText, this.inputThat, this.inputTopic,
                            this.reply);

                    // Reset all fields to defaults.
                    this.matchPattern = this.matchThat = this.matchTopic = ASTERISK;
                    this.inputText = this.inputThat = this.inputTopic = this.matchTemplate = this.reply = EMPTY_STRING;
