...
public class IRCListener implements Listener, ShellCommandable
{
    ...
    private boolean processMessageCommandClient(String command, String message)
    {
        ...
        if (command.equalsIgnoreCase("SERVER") || command.equalsIgnoreCase("CONNECT"))
        {
            ...
            else if (argc == 1)
            {
                ...
                if (this.clientStatus == CONNECTED)
                {
                    sendServerMessage("USER " + this.nick + " " + this.socket.getInetAddress().getHostName() + " server :" + this.nick);
            ...
            switch (Integer.parseInt(command))
            {
                ...
                case 322:
                {
                    ...
                    sendMessage(SERVERPREFIX, params.substring((firstSpaceIndex + 1), secondSpaceIndex) + ": "
                            + params.substring((secondSpaceIndex + 1), (colonIndex - 1)) + " " + params.substring((colonIndex + 1)));
                }
                    break;
     ...
     * @return the part of the string following the first <code>:</code>
     *         (colon), or the whole string if there is no colon.
     */
